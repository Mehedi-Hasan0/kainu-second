...
export default function FooterLinks() {
  return (
    <div className="main-container grid grid-cols-1 sm:grid-cols-2 md:flex md:flex-wrap md:flex-row lg:flex-nowrap justify-between gap-6">
      {/* Logos */}
      <div className="flex flex-col gap-3 md:gap-4">
        ...
        <p className="text-base xl:text-lg text-gray-300 font-poppins font-medium">
          ...
          <span className="uppercase text-csPrimary">Innovation Inc.</span>
        </p>
        ...
        <div className="mt-2 flex flex-col gap-4">
          {footerLink.quickLink.map((link) => (
            <Link
              href={"/"}
              key={link}
              className="text-gray-400 hover:text-csPrimary transition duration-300 ease-in-out font-poppins font-medium text-base xl:text-lg"
            >
              {link}
            </Link>
          ))}
